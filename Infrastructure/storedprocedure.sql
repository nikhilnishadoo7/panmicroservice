...
CREATE TABLE panresponsesjson (
    id BIGSERIAL PRIMARY KEY,
    correlation_id VARCHAR(100) NOT NULL,
    pan_verification_id BIGINT NOT NULL,
    request_id VARCHAR(100),
    encrypted_raw_response_json TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE FUNCTION insert_pan_verification(
    p_correlationid VARCHAR,
    p_masterid BIGINT,
    p_providerrequestid VARCHAR,
    p_panhash VARCHAR,
...
BEGIN
    ...
    VALUES (
        p_correlationid, p_masterid, p_providerrequestid,
        p_panhash, p_encryptedpan, p_panstatus, p_panlookupstatus,
        p_encryptedfullname, p_pancardtype, p_ispanaadhaarliked,
        p_callerip, p_createdat
    )
...
CREATE PROCEDURE insert_pan_response(
    p_correlation_id VARCHAR,
    p_pan_verification_id BIGINT,
    p_request_id VARCHAR,
    p_encrypted_raw_response_json TEXT,
    p_created_at TIMESTAMPTZ
)
...
CREATE FUNCTION get_active_providers()
RETURNS TABLE (
    id BIGINT,
    providername VARCHAR,
    baseurl VARCHAR,
...
BEGIN
    ...
    SELECT
        ...
        pm.provider_name,
        pm.provider_baseurl,
        pm.provider_endpoint,
        pm.encrypted_api_key,
        pm.priority,
        pm.is_active
    FROM providerpanmaster pm
    WHERE pm.is_active = true
...
INSERT INTO providerpanmaster
(provider_name, provider_baseurl, provider_endpoint, encrypted_api_key, priority)
VALUES
('sprintverify','http://localhost:3002','/api/v1/sprintverify','MOCK',1),
('surepass','https://kyc-api.surepass.io','/api/v1/pan/pan-adv-v2','eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..RBdnMnu1CIRb2flUzHNkzZYvOI-wN1CabDp_ZR4fXOQ',2);
DROP FUNCTION IF EXISTS get_pan_with_provider(TEXT);
CREATE OR REPLACE FUNCTION get_pan_with_provider(p_hash TEXT)
RETURNS TABLE (
    id BIGINT,
    correlationid VARCHAR,
    masterid BIGINT,
    panhash VARCHAR,
    encryptedpan VARCHAR,
    providerrequestid VARCHAR,
    panstatus VARCHAR,
    panlookupstatus VARCHAR,
    encryptedfullname VARCHAR,
    pancardtype VARCHAR,
    ispanaadhaarliked BOOLEAN,
    callerip VARCHAR,
    createdat TIMESTAMP,
